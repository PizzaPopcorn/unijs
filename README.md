...
## Setup
1. Install this repo as a package in your Unity project using the package manager. Open the package manager, click on the add button and then "Install package from git URL".
...
5. Build your game in WebGL and either copy it somewhere inside your wwwroot folder in your web page project, or upload it to a server.
6. In your web page project, import the JavaScript library on the html file that will load the Unity instace by adding this line:
```js
<script src="https://cdn.jsdelivr.net/npm/@pizzapopcorn/unityjs/dist/unity.min.js"></script>
```

7. Create another JavaScript file and also add it to the html page, then call `Unity.LoadInstance("url", "elementId");` where the url can be either an external url or the folder inside `wwwroot` where your build is located, and `elementId` is the name of the div where you want to contain your game canvas.
